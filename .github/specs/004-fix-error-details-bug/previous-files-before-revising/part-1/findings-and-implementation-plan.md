...
- A second root cause is schema mismatch: the current normalized UI error contract has no dedicated field for upstream identifiers (for example `model_not_found`), so those values are not reliably preserved or rendered.
...
- **Observed behavior**: Specific upstream identifiers (for example `model_not_found`) are not shown as first-class fields.
- **Likely cause**: Error schema does not model identifier/code/type explicitly across server/client boundary.
- **Evidence**:
  ...
  - `app/components/UiErrorAlert.vue` renders only `Type` (category), `Status Code`, `Details`; no `error code`/`identifier` field.
- **Impact**: Valuable machine-readable diagnostics are either discarded or buried in freeform text.
...
4. **Normalization (`app/utils/error-normalization.ts`)**
   ...
   - **Drop-off D**: No dedicated extraction path for upstream `code`/`type` fields (for example `model_not_found`).
...
6. **Rendering (`app/components/UiErrorAlert.vue`)**
   ...
   - **Drop-off E**: No render slot/field for upstream identifier/code/type.
...
- **Fields safe to show in UI (allowlist)**:
  ...
  - Stable, non-sensitive upstream identifier/code (for example `model_not_found`)
...
- **Acceptance criteria**:
  - UI shows `Type`, `Status Code` (when present), and `Error Code` (when present), with sanitized details.
...
### 6) Test Plan
- **T-001 Known typed API error (`model_not_found`)**:
  - Simulate upstream error containing status + code/type + message.
  - Verify normalized category = `api`, status displayed, code displayed, safe message shown.
