...
- Ensure the UI never displays user-facing `unknown` for type.
...
If `error.type` is absent (including unknown/unsupported formats), the UI may hide or omit the Type value and rely on sanitized stringified payload in `details`.
...
`app/app.vue` must guard `response.json()` with a try/catch. On parse failure, a fallback object `{ statusCode: response.status }` must still be forwarded to `normalizeUiError` so status is preserved and unknown/unsupported payload fallback can populate sanitized stringified `details`.
...
4. **Unknown error with status**: object without message but with `statusCode` → `category: "api"` if status is 4xx/5xx, and set sanitized stringified payload to `details` when possible.
5. **Truly unknown**: fallback → keep internal category behavior as needed, but set sanitized stringified payload to `details` and do not present user-facing `unknown` type text.
...
- **SR-7** (no user-facing unknown type): The UI must not render `unknown` as a Type value for unknown/unsupported errors; fallback diagnostics must be carried through sanitized `details`.
...
| `tests/unit/error-normalization.test.ts` | Unknown error object falls back to sanitized stringified payload in `details` |
...
| `tests/unit/ui-error-alert.test.ts` | Unknown/unsupported error does not render user-facing `unknown` Type value |
...
| `tests/e2e/app.spec.ts` | Unknown/untyped error payload → UI does not display `Type: unknown`; shows sanitized stringified fallback `details` |
...
| F-002 | Validation/Error Handling, Architecture | Parse/envelope mismatches must use sanitized stringified payload fallback |
...
- Unknown/unsupported errors do not display user-facing `Type: unknown` text.
