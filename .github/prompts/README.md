...
- **Step 5 — Implement from Plan**:
  ...
  - Requires each find-and-fix follow-up task to include `Severity` with one of: `critical`, `important`, or `minor`; appends tasks by severity order (`critical` → `important` → `minor`) while preserving discovery order within the same severity.
